            ...
            <div className="space-y-4">
              <div className="flex items-start gap-2 text-red-600 text-sm">
                ...
                <div className="flex-1">
                  {error || 'サーバーに接続できません。API が起動しているか確認してください。'}
                </div>
              </div>

              {configInfo && (
                <div className="space-y-2 text-xs text-muted-foreground border-t pt-3">
                  <div className="font-medium">診断情報:</div>
                  <div className="space-y-1 font-mono">
                    <div>バージョン: {configInfo.version}</div>
                    <div>ビルド時刻: {new Date(configInfo.buildTime).toLocaleString()}</div>
                    <div className="break-all">API URL: {configInfo.apiUrl}</div>
                    <div className="break-all">フロントエンド: {typeof window !== 'undefined' ? window.location.href : 'N/A'}</div>
                  </div>
                  <div className="text-xs pt-2">
                    詳細なログはブラウザーのコンソールで確認してください（🔧 [Config] を検索）
                  </div>
                </div>
              )}
